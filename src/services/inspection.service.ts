...
import { Prisma, TransactionType, TransactionCategory, ServiceGroup } from '@prisma/client';
...
const createInspection = async (
  ...
  const existingInspection = await prisma.roomInspection.findUnique({
    where: { stayDetailId }
  });
  ...
  if (totalPenalty.greaterThan(0)) {
    ...
    if (folio) {
      ...
      const penaltyServices = await prisma.service.findMany({
        where: {
      ...
      if (damageAmount.greaterThan(0)) {
        ...
        await prisma.folioTransaction.create({
          data: {
            ...
            description: `Property Damage - ${data.damageNotes || 'Room ' + stayDetail.room.code}`,
 ...
 */
const getInspectionByStayDetail = async (stayDetailId: number) => {
...
const updateInspection = async (
  ...
  const inspection = await prisma.roomInspection.update({
    where: { stayDetailId },
    data,
    include: {
      stayDetail: {
 ...
 */
const canCheckout = async (stayDetailId: number): Promise<boolean> => {
  const inspection = await prisma.roomInspection.findUnique({
    where: { stayDetailId }
  });
