...
import { Prisma, TransactionType, TransactionCategory, RoomStatus } from '@prisma/client';
 ...
 */
const getDailySnapshot = async (date: Date): Promise<DailySnapshot | null> => {
  const snapshotDate = new Date(date);
  snapshotDate.setHours(0, 0, 0, 0);

  const snapshot = await (prisma as any).dailySnapshot.findUnique({
    where: { snapshotDate }
  });
 ...
 */
const getSnapshotsInRange = async (fromDate: Date, toDate: Date): Promise<DailySnapshot[]> => {
  ...
  const snapshots = await (prisma as any).dailySnapshot.findMany({
    where: {
      snapshotDate: {
        gte: from,
        lte: to
 ...
 */
const getOccupancyReport = async (fromDate: Date, toDate: Date) => {
  const snapshots = await getSnapshotsInRange(fromDate, toDate);
 ...
 */
const getRevenueReport = async (fromDate: Date, toDate: Date) => {
  const snapshots = await getSnapshotsInRange(fromDate, toDate);
 ...
 */
const getBookingReport = async (fromDate: Date, toDate: Date) => {
  const snapshots = await getSnapshotsInRange(fromDate, toDate);
  ...
  const cancellationRate =
    totals.newReservations > 0 ? (totals.cancelledReservations / totals.newReservations) * 100 : 0;
...
const getDashboard = async () => {
  ...
  const todayDepartures = await prisma.stayDetail.count({
    where: {
  ...
  // Today's revenue
  const todayTransactions = await prisma.folioTransaction.aggregate({
    where: {
      postingDate: {
        gte: today,
        lte: endOfToday
  ...
  // Today's payments
  const todayPayments = await prisma.folioTransaction.aggregate({
    where: {
      postingDate: {
        gte: today,
        lte: endOfToday
  ...
  // Current guests
  const currentGuests = await prisma.guestInResidence.count({
    where: {
      stayDetail: {
        status: 'OCCUPIED'
      }
  ...
  const pendingHousekeeping = await prisma.housekeepingLog.count({
    where: {
  ...
  const yesterdaySnapshot: DailySnapshot | null = await (prisma as any).dailySnapshot.findUnique({
 ...
 */
const getRevenueByRoomType = async (fromDate: Date, toDate: Date) => {
  const from = new Date(fromDate);
  from.setHours(0, 0, 0, 0);
