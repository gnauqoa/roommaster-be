...
const createInvoice = async (
  ...
  const folio = await prisma.guestFolio.findUnique({
  ...
  const alreadyInvoiced = await prisma.invoiceDetail.findMany({
    where: {
 ...
 */
const queryInvoices = async (filter: {
  code?: string;
  guestFolioId?: number;
  invoiceToCustomerId?: number;
  employeeId?: number;
 ...
 */
const getInvoiceForPrint = async (invoiceId: number) => {
